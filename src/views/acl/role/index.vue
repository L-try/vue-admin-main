...
const setPermisstion = async (row: RoleData) => {
  drawer.value = true
  Object.assign(RoleParams, row)
  let result: any = await reqAllMenuList(RoleParams.id as number) // 临时用any，后续可定义接口类型
  if (result.code == 200) {
    // 从对象中提取allPermissions数组
    menuArr.value = result.data.allPermissions
    console.log('用于树形组件的数组:', menuArr.value)
    // 此时menuArr是数组，可正确渲染
...
// 修改后的函数，兼容首次调用的对象和递归时的数组
const filterSelectArr = (data: any, initArr: any) => {
  // 确定要遍历的数组：如果是对象且有allPermissions属性，则使用它，否则直接使用data（数组）
  const items =
    typeof data === 'object' && !Array.isArray(data) && data.allPermissions
      ? data.allPermissions
      : data

  // 确保items是数组再进行遍历
  if (!Array.isArray(items)) {
    console.warn('传入的数据不是有效的数组或包含allPermissions的对象', data)
    return initArr
  }
